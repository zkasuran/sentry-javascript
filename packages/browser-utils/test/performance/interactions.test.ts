...
describe('interactionsIntegration', () => {
  ...
  function getInteractionSpans(): Span[] {
    return endedSpans.filter(span => spanToJSON(span).op === 'ui.action.click');
  }
  ...
  it('inherits the source of the route span', () => {
    ...
    expect(spanToJSON(getInteractionSpans()[0]!).data).toMatchObject({ 'sentry.source': 'route' });
  });
  ...
  it('falls back to a url source when the route span has none', () => {
    ...
    expect(spanToJSON(getInteractionSpans()[0]!).data).toMatchObject({ 'sentry.source': 'url' });
  });
  ...
  it('picks up the route name a router set after the route span started', () => {
    ...
    expect(spanToJSON(getInteractionSpans()[0]!).description).toBe('/users/:id');
  });
  ...
  it('interrupts a still running interaction span when the next click happens', () => {
    ...
    expect(spanToJSON(spans[0]!).data).toMatchObject({
      'sentry.idle_span_finish_reason': 'interactionInterrupted',
    });
    expect(spanToJSON(spans[1]!).data).toMatchObject({ 'sentry.idle_span_finish_reason': 'idleTimeout' });
  });
